...
```{r}
FileList2 = sapply(RNADirectoryList, 
                   function(x){list.files(path = x, 
                                          full.names = TRUE, 
                                          pattern = "salmon.txt") }) 

FileIDList2 <- as.data.frame(cbind(FileList2, 
                                  gsub("^.*-","", RNADirectoryList)), 
                            stringsAsFactors = F)
colnames(FileIDList2) <- c("Path", "molecular_id")
head(FileIDList2, n = 3)
...
listOf_alldf2 <- lapply(seq(1:nrow(FileIDList2)),
                 function(i){ 
                   X <- read.delim(file = FileIDList2$Path[i],
                                         header = FALSE);
                   colnames(X) <- c("Gene", FileIDList2$molecular_id[i]);
                   return(X)
                   } )
...
#all_joined_df2 <- plyr:: join_all(listOf_alldf2, by = NULL, 
#                                 type = "full", match = "all")
#head(all_joined_df2, n = 3)
```
