...
- What groups 2 analyse (to be filled in later) - why i chose them/why they important?
...
- Explain the reasoning for both the difference in the bio vs. tech factors /// why im picking each of the columns within those sections
Will separate demographic groups by phenotype, and compare gene expression counts between the
two groups, groups defined by conditions of that factor
(example:
age:  - conditions of = age groups, 
sex = male or female,
genotype = present or not present).
group 1:  male,
group 2, female, (one or the other)
in each group, compare gene counts for each gene.(aka counts of RNA molecules/expression of genes) 
ultimate goal is to make observations/conclusions about possible differences in gene counts between different groups that may or may not be significant.
# Data Import
## Install R Packages
...
```{r}
#dplyr:: group_by(all_joined_df, "Gene", "molecular_id")
dplyr:: group_by(df_combined, "sex", "molecular_id")
```
...
```{r Load Additional Libraries, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
