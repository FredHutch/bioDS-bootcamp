...
venn_summary <- summarize(df_htseqNsalmonLong, 
                          HTonly = sum((!is.na(htseq_counts)) & (is.na(salmon_counts))),
                          Salmononly = sum((is.na(htseq_counts)) & (!is.na(salmon_counts))),
...
#QuickMathSlol
venn_summary$total=sum(venn_summary[1,1:4])
#Creates new column                   Asks if 'total' column = number of rows of original DF
venn_summary$is_same_as_num_of_rows_of_df <- venn_summary[1,5] == ( nrow(df_htseqNsalmonLong) )
print(venn_summary[1,6])
